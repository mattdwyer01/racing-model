...
Output: run_id, race_id, snap_utc, trr_pre, pfm_pre.
"""
import argparse
import io
import subprocess
...
def _read(repo, sha):
    ...
    want = [c for c in ["run_id", "race_id", "date", "toprate_rating", "pfm_score"] if c in head]
...
def main():
    ...
    s[["run_id", "race_id", "snap_utc", "trr_pre", "pfm_pre"]].to_csv(OUT, index=False, compression="gzip")
    print(f"{len(s):,} runners -> {OUT}")
